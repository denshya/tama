...
// ─── CSS helper (module-level, once) ────

function toStyleObj(cssText: string): Record<string, string | number> {
  ...
  for (const rule of cssText.split(";")) {
    ...
    if (colon === -1) continue
    const key = trimmed.slice(0, colon).trim().replace(/-([a-z])/g, (_, c: string) => c.toUpperCase())
    obj[key] = trimmed.slice(colon + 1).trim()
  }
  return obj
}

// Wrapper element style objects (shared across component-tree factories)
const _mainStyle = { display: "flex", gap: "2.5em", maxWidth: "1300px", margin: "0 auto", padding: "1em" } as const
const _asideStyle = { minWidth: "15em", position: "sticky", top: "4em", alignSelf: "flex-start", maxHeight: "calc(100vh - 5em)", overflowY: "auto" } as const
const _articleStyle = { flex: 1, minWidth: 0 } as const

// ─── String-style component variants ────
...
// ─── Style-object static component variants ────

function Sidebar_obj() {
  return (
    <nav className="sidebar">
      <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
        <li style={{ marginBottom: "0.25em" }}>
          <span style={{ display: "flex", alignItems: "center", gap: "0.25em", padding: "0.375em 0.5em", borderRadius: "0.375em", cursor: "pointer", fontWeight: 600, fontSize: "0.875rem", color: "#171717" }}>
            <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={{ transition: "transform 0.2s" }}><path d="M6 4l4 4-4 4" /></svg>
            Getting Started
          </span>
          <ul style={{ listStyle: "none", padding: 0, margin: "0 0 0 1.25em" }}>
            <li><a href="/learn" style={{ display: "block", padding: "0.375em 0.5em", borderRadius: "0.375em", fontSize: "0.875rem", color: "#171717", textDecoration: "none", background: "rgba(0,0,0,0.05)" }}>Learn</a></li>
            <li><a href="/installation" style={{ display: "block", padding: "0.375em 0.5em", borderRadius: "0.375em", fontSize: "0.875rem", color: "#666", textDecoration: "none" }}>Installation</a></li>
            <li><a href="/quick-start" style={{ display: "block", padding: "0.375em 0.5em", borderRadius: "0.375em", fontSize: "0.875rem", color: "#666", textDecoration: "none" }}>Quick Start</a></li>
          </ul>
        </li>
        <li style={{ marginBottom: "0.25em" }}>
          <a href="/guide" style={{ display: "flex", alignItems: "center", gap: "0.25em", padding: "0.375em 0.5em", borderRadius: "0.375em", fontSize: "0.875rem", color: "#171717", textDecoration: "none" }}>Guide</a>
        </li>
        <li style={{ marginBottom: "0.25em" }}>
          <span style={{ display: "flex", alignItems: "center", gap: "0.25em", padding: "0.375em 0.5em", borderRadius: "0.375em", cursor: "pointer", fontWeight: 600, fontSize: "0.875rem", color: "#171717" }}>
            <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={{ transition: "transform 0.2s" }}><path d="M6 4l4 4-4 4" /></svg>
        ...
        <li style={{ marginBottom: "0.25em" }}>
          <a href="/examples" style={{ display: "flex", alignItems: "center", gap: "0.25em", padding: "0.375em 0.5em", borderRadius: "0.375em", fontSize: "0.875rem", color: "#171717", textDecoration: "none" }}>Examples</a>
        </li>
        <li style={{ marginBottom: "0.25em" }}>
          <a href="/blog" style={{ display: "flex", alignItems: "center", gap: "0.25em", padding: "0.375em 0.5em", borderRadius: "0.375em", fontSize: "0.875rem", color: "#666", textDecoration: "none" }}>Blog</a>
        </li>
      </ul>
    </nav>
  )
  ...
  return (
    <nav className="breadcrumbs" style={{ marginBottom: "1em", fontSize: "0.875rem", color: "#666" }}>
      <ol style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", alignItems: "center", gap: "0.5em" }}>
        ...
        <li style={{ display: "flex", alignItems: "center", gap: "0.5em" }}>
          <span style={{ color: "#171717", background: "rgba(0,0,0,0.05)", padding: "0.125em 0.375em", borderRadius: "0.25em" }}>Getting Started</span>
        </li>
      </ol>
    </nav>
  )
...
// ─── State + Events component variants (no classMods) ────

function Sidebar_noClassMods(props: { activeRoute: State<string>, expandedFolders: State<Set<string>> }) {
  const isLearnActive = props.activeRoute.is("learn")
  const isGuideActive = props.activeRoute.is("guide")
  const isExamplesActive = props.activeRoute.is("examples")
  const isBlogActive = props.activeRoute.is("blog")
  ...
  return (
    <nav className="sidebar">
      <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
        <li style={{ marginBottom: "0.25em" }}>
          <span style={{ display: "flex", alignItems: "center", gap: "0.25em", padding: "0.375em 0.5em", borderRadius: "0.375em", cursor: "pointer", fontWeight: 600, fontSize: "0.875rem", color: "#171717" }} onclick={toggleGS}>
            <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={{ transition: "transform 0.2s" }}><path d="M6 4l4 4-4 4" /></svg>
            Getting Started
          </span>
          <ul style={{ listStyle: "none", padding: 0, margin: "0 0 0 1.25em" }}>
        ...
        <li style={{ marginBottom: "0.25em" }}>
          <span style={{ display: "flex", alignItems: "center", gap: "0.25em", padding: "0.375em 0.5em", borderRadius: "0.375em", cursor: "pointer", fontWeight: 600, fontSize: "0.875rem", color: "#171717" }} onclick={toggleApi}>
            <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={{ transition: "transform 0.2s" }}><path d="M6 4l4 4-4 4" /></svg>
            API Reference
          </span>
          <ul style={{ listStyle: "none", padding: 0, margin: "0 0 0 1.25em" }}>
...
// ─── Incremental factory functions ────

export function createTamaJSX_styleObjects() {
  const topbarStyle = toStyleObj("position: sticky; top: 0; z-index: 100;")
  const containerStyle = toStyleObj("max-width: 1300px; margin: 0 auto; display: flex; align-items: center; padding: 0.5em 1em;")
  const logoSpanStyle = toStyleObj("font-weight: 700; font-size: 1.125rem;")
  const navStyle = toStyleObj("flex: 1; display: flex; justify-content: center;")
  const secLinksStyle = toStyleObj("display: flex; gap: 0.75em; align-items: center;")
  const socialLinkStyle = toStyleObj("color: #666;")
  const mainStyle = toStyleObj("display: flex; gap: 2.5em; max-width: 1300px; margin: 0 auto; padding: 1em;")
  const asideStyle = toStyleObj("min-width: 15em; position: sticky; top: 4em; align-self: flex-start; max-height: calc(100vh - 5em); overflow-y: auto;")
  const ulStyle = toStyleObj("list-style: none; padding: 0; margin: 0;")
  const liStyle = toStyleObj("margin-bottom: 0.25em;")
  const sidebarItemStyle = toStyleObj("display: flex; align-items: center; gap: 0.25em; padding: 0.375em 0.5em; border-radius: 0.375em; cursor: pointer; font-weight: 600; font-size: 0.875rem; color: #171717;")
  const sidebarNestedUlStyle = toStyleObj("list-style: none; padding: 0; margin: 0 0 0 1.25em;")
  const sidebarActiveLinkStyle = toStyleObj("display: block; padding: 0.375em 0.5em; border-radius: 0.375em; font-size: 0.875rem; color: #171717; text-decoration: none; background: rgba(0,0,0,0.05);")
  const sidebarLinkStyle = toStyleObj("display: block; padding: 0.375em 0.5em; border-radius: 0.375em; font-size: 0.875rem; color: #666; text-decoration: none;")
  const sidebarLinkFlexStyle = toStyleObj("display: flex; align-items: center; gap: 0.25em; padding: 0.375em 0.5em; border-radius: 0.375em; font-size: 0.875rem; color: #171717; text-decoration: none;")
  const articleStyle = toStyleObj("flex: 1; min-width: 0;")
  const breadcrumbNavStyle = toStyleObj("margin-bottom: 1em; font-size: 0.875rem; color: #666;")
  const breadcrumbOlStyle = toStyleObj("list-style: none; padding: 0; margin: 0; display: flex; align-items: center; gap: 0.5em;")
  const breadcrumbLiStyle = toStyleObj("display: flex; align-items: center; gap: 0.5em;")
  const breadcrumbLinkStyle = toStyleObj("color: #666; text-decoration: none;")
  const breadcrumbCurrentStyle = toStyleObj("color: #171717; background: rgba(0,0,0,0.05); padding: 0.125em 0.375em; border-radius: 0.25em;")
  const contentStyle = toStyleObj("line-height: 1.5;")
  const h1Style = toStyleObj("font-size: 2.25rem; font-weight: 700; margin: 0 0 0.5em; color: #171717;")
  const pStyle = toStyleObj("margin: 0 0 0.75em; color: #333;")
  const h2Style = toStyleObj("font-size: 1.5rem; font-weight: 600; margin: 1.5em 0 0.5em; color: #171717;")
  const codeBlockStyle = toStyleObj("background: #f6f8fa; border-radius: 0.5em; overflow: hidden; margin-bottom: 1em;")
  const codeHeaderStyle = toStyleObj("padding: 0.5em 1em; font-size: 0.8rem; color: #666; border-bottom: 1px solid #e1e4e8; font-family: monospace;")
  const preStyle = toStyleObj("padding: 1em; margin: 0; overflow-x: auto;")
  const codeStyle = toStyleObj("font-family: monospace; font-size: 0.875rem; line-height: 1.4;")
  const inlineCodeStyle = toStyleObj("background: #f6f8fa; padding: 0.125em 0.375em; border-radius: 0.25em; font-size: 0.85em;")
  const tipStyle = toStyleObj("margin: 1em 0; padding: 0.75em 1em; border-left: 4px solid #2b82d9; background: #f0f7ff; border-radius: 0.375em;")
  const noteStyle = toStyleObj("margin: 1em 0; padding: 0.75em 1em; border-left: 4px solid #6b7280; background: #f9fafb; border-radius: 0.375em;")
  const strongStyle = toStyleObj("color: #171717;")
  const ulListStyle = toStyleObj("margin: 0 0 0.75em; padding-left: 1.5em; color: #333;")
  const liSmallStyle = toStyleObj("margin-bottom: 0.375em;")
  const svgIconStyle = toStyleObj("margin-right: 0.375em;")
  const svgChevronStyle = toStyleObj("transition: transform 0.2s;")
  const linkBlueStyle = toStyleObj("color: #2b82d9; text-decoration: none;")
  const footerStyle = toStyleObj("border-top: 1px solid #e1e4e8; margin-top: 3em; background: #f6f8fa;")
  const footerTopStyle = toStyleObj("max-width: 1300px; margin: 0 auto; padding: 1.5em 1em; display: flex; justify-content: space-between; align-items: center;")
  const footerLogoDivStyle = toStyleObj("display: flex; align-items: center; gap: 0.5em;")
  const footerLogoSpanStyle = toStyleObj("font-weight: 600;")
  const footerLinksDivStyle = toStyleObj("display: flex; gap: 1.5em;")
  const footerLinkAStyle = toStyleObj("color: #666; text-decoration: none; font-size: 0.875rem;")
  const footerBottomStyle = toStyleObj("max-width: 1300px; margin: 0 auto; padding: 1em; border-top: 1px solid #e1e4e8; display: flex; justify-content: space-between; align-items: center; font-size: 0.8rem; color: #666;")
  const footerLegalStyle = toStyleObj("display: flex; gap: 1em;")

  return (
    <div id="app-root">
      <header id="topbar" style={topbarStyle}>
        <div className="container" style={containerStyle}>
          <a href="/" className="logo" style={toStyleObj("display: flex; align-items: center; gap: 0.5em; text-decoration: none; color: inherit;")}>
            <svg viewBox="0 0 25 25" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" />
            </svg>
            <span style={logoSpanStyle}>Tama</span>
          </a>
          <nav className="navbar" style={navStyle} />
          <div className="secondary-links" style={secLinksStyle}>
            <a href="https://github.com" aria-label="GitHub" style={socialLinkStyle}>
              <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.604-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z" /></svg>
            </a>
            <a href="https://discord.com" aria-label="Discord" style={socialLinkStyle}>
              <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M20.317 4.37a19.791 19.791 0 00-4.885-1.515.074.074 0 00-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 00-5.487 0 12.64 12.64 0 00-.617-1.25.077.077 0 00-.079-.037A19.736 19.736 0 003.677 4.37a.07.07 0 00-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 00.031.057 19.9 19.9 0 005.993 3.03.078.078 0 00.084-.028 14.09 14.09 0 001.226-1.994.076.076 0 00-.041-.106 13.107 13.107 0 01-1.872-.892.077.077 0 01-.008-.128 10.2 10.2 0 00.372-.292.074.074 0 01.077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 01.078.01c.12.098.246.198.373.292a.077.077 0 01-.006.127 12.299 12.299 0 01-1.873.892.077.077 0 00-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 00.084.028 19.839 19.839 0 006.002-3.03.077.077 0 00.032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 00-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" /></svg>
            </a>
            <a href="https://bsky.app" aria-label="Bluesky" style={socialLinkStyle}>
              <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 10.8c-1.087-2.114-4.046-6.053-6.798-7.995C2.566 1.023 0 1.62 0 4.2c0 .84.48 7.08.756 8.1.972 3.468 4.512 4.344 7.656 3.792-5.352 1.152-6.72 4.296-3.864 6.096 5.712 3.6 7.452-2.076 7.452-6.396V4.956c0-.42-.336-.756-.756-.756-2.52 0-5.412 4.776-6.588 6.636z" /></svg>
            </a>
          </div>
        </div>
      ...
      <main style={mainStyle}>
        <aside style={asideStyle}>
          <nav className="sidebar">
            <ul style={ulStyle}>
              <li style={liStyle}>
                <span style={sidebarItemStyle}>
                  <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={svgChevronStyle}><path d="M6 4l4 4-4 4" /></svg>
                  Getting Started
                </span>
                <ul style={sidebarNestedUlStyle}>
                  <li><a href="/learn" style={sidebarActiveLinkStyle}>Learn</a></li>
                  <li><a href="/installation" style={sidebarLinkStyle}>Installation</a></li>
                  <li><a href="/quick-start" style={sidebarLinkStyle}>Quick Start</a></li>
                </ul>
              </li>
              <li style={liStyle}>
                <a href="/guide" style={sidebarLinkFlexStyle}>Guide</a>
              </li>
              <li style={liStyle}>
                <span style={sidebarItemStyle}>
                  <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={svgChevronStyle}><path d="M6 4l4 4-4 4" /></svg>
                ...
                <ul style={sidebarNestedUlStyle}>
                  <li><a href="/api/state" style={sidebarLinkStyle}>State</a></li>
                  <li><a href="/api/component" style={sidebarLinkStyle}>Component</a></li>
                  <li><a href="/api/inflator" style={sidebarLinkStyle}>Inflator</a></li>
                  <li><a href="/api/jsx" style={sidebarLinkStyle}>JSX</a></li>
                </ul>
              </li>
              <li style={liStyle}>
                <a href="/examples" style={sidebarLinkFlexStyle}>Examples</a>
              </li>
              <li style={liStyle}>
        ...
        <article style={articleStyle}>
          <nav className="breadcrumbs" style={breadcrumbNavStyle}>
            <ol style={breadcrumbOlStyle}>
              <li style={breadcrumbLiStyle}>
                <a href="/" style={breadcrumbLinkStyle}>Home</a>
                <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor"><path d="M6 4l4 4-4 4" /></svg>
              </li>
              <li style={breadcrumbLiStyle}>
                <a href="/learn" style={breadcrumbLinkStyle}>Learn</a>
                <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor"><path d="M6 4l4 4-4 4" /></svg>
              </li>
              <li style={breadcrumbLiStyle}>
                <span style={breadcrumbCurrentStyle}>Getting Started</span>
              </li>
            </ol>
          </nav>
          <div className="md-content" style={contentStyle}>
            <h1 style={h1Style}>Getting Started</h1>
            <p style={pStyle}>Welcome to Tama! This guide will help you get started building applications with fine-grained reactivity and direct DOM rendering.</p>
            <h2 style={h2Style}>Install</h2>
            <p style={pStyle}>Run the following command in your terminal:</p>
            <div className="code-block" style={codeBlockStyle}>
              <div className="code-header" style={codeHeaderStyle}>Terminal</div>
              <pre style={preStyle}><code style={codeStyle}>bun i @denshya/proton @denshya/reactive</code></pre>
            </div>
            <h2 style={h2Style}>Setup</h2>
            <p style={pStyle}>Configure your <code style={inlineCodeStyle}>tsconfig.json</code> to use Tama as the JSX runtime:</p>
            <div className="code-block" style={codeBlockStyle}>
              <div className="code-header" style={codeHeaderStyle}>tsconfig.json</div>
              <pre style={preStyle}><code style={codeStyle}>{`{
  "compilerOptions": {
    "jsx": "react-jsx",
    "jsxImportSource": "@denshya/proton/jsx/virtual"
  }
            ...
            <p style={pStyle}>Here&apos;s a simple counter app to get you started:</p>
            <div className="code-block" style={codeBlockStyle}>
              <div className="code-header" style={codeHeaderStyle}>App.tsx</div>
              <pre style={preStyle}><code style={codeStyle}>{`import { State } from "@denshya/reactive"
import { WebInflator } from "@denshya/proton"
  ...
  inflator.inflate(
    <button onclick={() => count.set(count.get() + 1)}>
      Count: {count}
            ...
            <blockquote className="admonition tip" style={tipStyle}>
              <strong style={strongStyle}>TIP:</strong> Tama directly renders to real DOM elements — no virtual DOM overhead. Each reactive binding updates only the specific DOM nodes that changed.
            </blockquote>
            <blockquote className="admonition note" style={noteStyle}>
              <strong style={strongStyle}>NOTE:</strong> Components in Tama are not functions — they are JSX trees. There are no hooks, no diffing, and no reconciliation algorithm.
            </blockquote>
            <h2 style={h2Style}>React Developers: The Short Version</h2>
            <ul style={ulListStyle}>
              <li style={liSmallStyle}><svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={svgIconStyle}><path d="M6 4l4 4-4 4" /></svg>No virtual DOM — direct DOM manipulation via WebInflator</li>
              <li style={liSmallStyle}><svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={svgIconStyle}><path d="M6 4l4 4-4 4" /></svg>Components are JSX trees, not function calls</li>
              <li style={liSmallStyle}><svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={svgIconStyle}><path d="M6 4l4 4-4 4" /></svg>State management via <code style={inlineCodeStyle}>@denshya/reactive</code></li>
              <li style={liSmallStyle}><svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={svgIconStyle}><path d="M6 4l4 4-4 4" /></svg>Fine-grained reactivity — only changed nodes update</li>
              <li style={liSmallStyle}><svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={svgIconStyle}><path d="M6 4l4 4-4 4" /></svg>No hooks — just state bindings and JSX</li>
              <li style={liSmallStyle}><svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" style={svgIconStyle}><path d="M6 4l4 4-4 4" /></svg>Built for performance from the ground up</li>
            </ul>
            <h2 style={h2Style}>Next Steps</h2>
            <p style={pStyle}>Continue reading to learn more about:</p>
            <ul style={ulListStyle}>
              <li style={liSmallStyle}><a href="/guide/reactivity" style={linkBlueStyle}>Reactivity</a> — Understanding fine-grained state management</li>
              <li style={liSmallStyle}><a href="/guide/components" style={linkBlueStyle}>Components</a> — Building reusable JSX trees</li>
              <li style={liSmallStyle}><a href="/guide/inflation" style={linkBlueStyle}>Inflation</a> — How JSX becomes DOM</li>
              <li style={liSmallStyle}><a href="/examples" style={linkBlueStyle}>Examples</a> — Real-world usage patterns</li>
            </ul>
          </div>
        </article>
      </main>
      <footer style={footerStyle}>
        <div className="footer-top" style={footerTopStyle}>
          <div className="footer-logo" style={footerLogoDivStyle}>
            <svg viewBox="0 0 25 25" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" />
            </svg>
            <span style={footerLogoSpanStyle}>Tama</span>
          </div>
          <div className="footer-links" style={footerLinksDivStyle}>
            <a href="/blog" style={footerLinkAStyle}>Blog</a>
            <a href="/schedule" style={footerLinkAStyle}>Schedule</a>
          </div>
        </div>
        <div className="footer-bottom" style={footerBottomStyle}>
          <span>&copy; 2025 Denshya. All rights reserved.</span>
          <div className="footer-legal" style={footerLegalStyle}>
            <a href="/privacy" style={footerLinkAStyle}>Privacy</a>
            <a href="/terms" style={footerLinkAStyle}>Terms</a>
            <a href="/cookies" style={footerLinkAStyle}>Cookies</a>
          </div>
        </div>
      </footer>
    </div>
...
// ─── Component-tree incremental factories ────

export function createTamaJSXFull_componentsOnly() {
  return (
    <div id="app-root">
      <Topbar_str />
      <main style="display: flex; gap: 2.5em; max-width: 1300px; margin: 0 auto; padding: 1em;">
...
export function createTamaJSXFull_componentsStyleObjects() {
  return (
    <div id="app-root">
      <Topbar />
...
export function createTamaJSXFull_componentsStateEvents() {
  const activeRoute = new State("learn")
  const expandedFolders = new State(new Set(["getting-started"]))
  const breadcrumbPath = new State("learn/getting-started")
  ...
  return (
    <div id="app-root">
      ...
      <main style={_mainStyle}>
        <aside style={_asideStyle}>
          <Sidebar_noClassMods activeRoute={activeRoute} expandedFolders={expandedFolders} />
        </aside>
        <article style={_articleStyle}>
          <Breadcrumbs_ breadcrumbPath={breadcrumbPath} />
          <PageContent />
...
export function createTamaJSXFull_componentsClassMods() {
  const activeRoute = new State("learn")
  const expandedFolders = new State(new Set(["getting-started"]))
  const breadcrumbPath = new State("learn/getting-started")
